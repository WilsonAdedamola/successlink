...
const Home = () => {
  return (
    <div className="bg min-h-screen px-4">
      <div className="2xl:max-w-[70%] 2xl:gap-9 flex flex-col justify-center pt-10 gap-4 mx-auto sm:px-7 md:flex-row md:items-center md:justify-between lg:pt-20">
        <p className="flex flex-col gap-2 items-start font-bold text-3xl sm:text-4xl md:text-6xl lg:text-7xl">GO Explore, <span>It's a big world</span> <span>Out there</span></p>
        <p className="flex flex-col items-start self-end sm:text-xl md:self-">
          "Man cannot discover new oceans <span>unless he has </span><span>the courage to lose
          sight of</span> the shore"
      ...
      <div className="flex flex-col items-center gap-4 justify-between rounded mt-7 bg-white px-5 py-3 md:px-2 md:mt-32 md:flex-row lg:max-w-[80%] mx-auto 2xl:mt-60">
        ...
        <button className="bg-[#992288] text-xs p-2 rounded-3xl w-72">See more</button>
      </div>
    </div>
  );
};
